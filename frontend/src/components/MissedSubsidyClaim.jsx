...
export function MissedSubsidyClaim({ translations, defaultState }) {
  ...
  // Diagnostic state from backend
  const [reasons, setReasons] = useState(subsidyRejectionReasons)
  ...
  // 1. Fetch grievance reasons on mount
  useEffect(() => {
    let isMounted = true
    fetch('/api/grievance/reasons')
      .then(res => {
        if (!res.ok) throw new Error(`HTTP ${res.status}`)
        return res.json()
      })
      .then(data => {
        if (isMounted && data.success && Array.isArray(data.reasons)) {
          setReasons(data.reasons)
        }
      })
      .catch(() => {
        // Fallback to static subsidyRejectionReasons silently
      })
  ...
  // 2. Submit Claim via POST /api/grievance/claim
  const handleClaimSubmit = (e) => {
    e.preventDefault()
    if (!consumerNo.trim() || !installDate) {
      setSubmitError('Please fill in your Consumer Number and Installation Date.')
      return
    }
    ...
    fetch('/api/grievance/claim', {
      ...
      body: JSON.stringify({
        ...
        issueType,
        remarks: `Escalation lodged for ${issueType} under ${stateName} DISCOM.`
      })
    })
      .then(res => {
        if (!res.ok) throw new Error(`HTTP ${res.status}`)
      ...
      .then(data => {
        if (data.success && data.claim) {
          setGeneratedTicket(data.claim)
          setClaimSubmitted(true)
          setTrackedResult(data.claim)
        } else {
          setSubmitError(data.error || data.message || 'Failed to submit claim.')
        }
      })
      .catch(() => {
        setSubmitError('Grievance server unreachable. Please verify the backend is running at http://localhost:5000.')
      })
      .finally(() => {
        setSubmitting(false)
      })
  ...
  // 3. Track Claim via GET /api/grievance/track/:query
  const handleTrackSearch = (e) => {
    ...
    fetch(`/api/grievance/track/${encodeURIComponent(q)}`)
      .then(res => {
        if (!res.ok) throw new Error(`HTTP ${res.status}`)
        return res.json()
      })
      .then(data => {
        if (data.success && data.claim) {
          ...
          setTrackError(data.message || 'No grievance record found for this tracking number.')
        }
      })
      .catch(() => {
        setTrackError('Tracking server unreachable. Please verify backend connection.')
      })
      .finally(() => {
        setTracking(false)
      })
  ...
  const currentRoadblock = reasons.find(r => r.id === selectedIssue) || reasons[0] || subsidyRejectionReasons[0]
          ...
          <div className="claim-form-view">
            {!claimSubmitted ? (
              <form className="claim-form-card" onSubmit={handleClaimSubmit}>
                ...
                {submitError && (
                  <div style={{ padding: '10px 14px', background: '#fee2e2', color: '#b91c1c', borderRadius: '8px', fontSize: '13px', marginBottom: '16px' }}>
                    ⚠️ {submitError}
                  </div>
                )}

                <div className="form-actions">
                  <button type="submit" className="btn-submit-claim" disabled={submitting}>
                    <span>{submitting ? 'Submitting Escalation Dossier...' : 'Generate & Submit Escalation Dossier'}</span>
            ...
            <form className="track-search-bar" onSubmit={handleTrackSearch}>
              <div className="search-input-wrap">
                <input 
                  ...
                  disabled={tracking}
                />
                <button type="submit" className="track-submit-btn" disabled={tracking || !trackQuery.trim()}>
                  <span>{tracking ? 'Tracking...' : 'Track Status'}</span>
                  <span>→</span>
                </button>
              </div>
              {trackError && (
                <div style={{ padding: '10px 14px', background: '#fee2e2', color: '#b91c1c', borderRadius: '8px', fontSize: '13px', marginTop: '12px' }}>
                  ⚠️ {trackError}
                </div>
              )}
            </form>
            ...
            {trackedResult && (
              <div className="tracker-card">
                ...
                <div className="progress-steps-row">
                  {trackedResult.steps && Array.isArray(trackedResult.steps) ? (
                    trackedResult.steps.map((st, sIdx) => {
                      const stepVal = trackedResult.currentStep || trackedResult.step || 2
                      const isCompleted = st.completed || stepVal > st.step
                      const isActive = stepVal === st.step && !st.completed
                      return (
                        <div key={st.step || sIdx} style={{ display: 'contents' }}>
                          <div className={`step-item ${isCompleted ? 'completed' : isActive ? 'active' : ''}`}>
                            <div className="step-circle">{isCompleted ? '✓' : st.step}</div>
                            <span className="step-label">{st.title}</span>
                            <span className="step-date">{st.date}</span>
                          </div>
                          {sIdx < trackedResult.steps.length - 1 && (
                    ...
                    <>
                      <div className={`step-item ${(trackedResult.currentStep || trackedResult.step || 2) >= 1 ? 'completed' : ''}`}>
                        <div className="step-circle">✓</div>
                        <span className="step-label">Claim Lodged</span>
                        <span className="step-date">{trackedResult.submissionDate}</span>
                      </div>
                      <div className={`step-connector ${(trackedResult.currentStep || trackedResult.step || 2) >= 2 ? 'completed' : ''}`}></div>
                      <div className={`step-item ${(trackedResult.currentStep || trackedResult.step || 2) >= 2 ? 'active' : ''}`}>
                        <div className="step-circle">2</div>
                        <span className="step-label">DISCOM & Vendor Review</span>
                        <span className="step-date">In Progress</span>
                      </div>
                      <div className={`step-connector ${(trackedResult.currentStep || trackedResult.step || 2) >= 3 ? 'completed' : ''}`}></div>
                      <div className={`step-item ${(trackedResult.currentStep || trackedResult.step || 2) >= 3 ? 'active' : ''}`}>
                        <div className="step-circle">3</div>
                        <span className="step-label">NPCI / Bank Re-mapping</span>
                        <span className="step-date">Pending</span>
                      </div>
                      <div className={`step-connector ${(trackedResult.currentStep || trackedResult.step || 2) >= 4 ? 'completed' : ''}`}></div>
                      <div className={`step-item ${(trackedResult.currentStep || trackedResult.step || 2) >= 4 ? 'active' : ''}`}>
                ...
                <div className="tracker-advice-box">
                  ...
                  <p>
                    {trackedResult.nodalOfficerRemarks || (
                      `Your escalation has been routed to the ${trackedResult.stateName} DISCOM Nodal Officer. If your bank account was unseeded, please ensure you submit the NPCI Aadhaar mandate at your home bank branch within 5 working days.`
                    )}
                  </p>
                </div>
